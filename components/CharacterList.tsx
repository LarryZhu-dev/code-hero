...
import { getRoleDisplayName, classifyHero } from '../utils/heroSystem';
import HeroAvatar from './HeroAvatar';
import { IconBack, IconDownload, IconEdit, IconPlay, IconPlus, IconTrash, IconSword } from './PixelIcons';
        ...
        <div className="h-full w-full p-8 bg-slate-900 flex flex-col">
            ...
            {heroes.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-y-auto pb-20 custom-scrollbar p-4">
                    {heroes.map(hero => {
                        const roleName = hero.role ? getRoleDisplayName(hero.role).split('(')[0] : '战士';
                        ...
                        return (
                            <div 
                                ...
                                className={`pixel-card group relative cursor-pointer hover:bg-slate-700 flex flex-col gap-3 border-4 ${mode === 'SELECT' ? 'hover:border-green-500' : ''}`}
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="flex-1 text-right flex flex-col items-end">
                                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">定位</div>
                                        <div className="text-xs text-blue-300 font-bold bg-blue-900/30 px-2 py-1 inline-flex items-center justify-center border-2 border-blue-900">
                                            {roleName}
                                        </div>
                                    </div>
                                </div>
                                ...
                                {mode === 'MANAGE' && (
                                    <div className="flex gap-2 mt-auto pt-2 border-t-2 border-slate-700/50">
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); onEdit(hero); }}
                                            className="flex-1 pixel-btn pixel-btn-secondary py-1 text-xs flex items-center justify-center gap-2 border-2"
                                        >
                                            <IconEdit size={12} /> 编辑
                                        </button>
                                        <button 
                                            onClick={(e) => handleDelete(hero.id, e)}
                                            className="pixel-btn pixel-btn-danger py-1 px-3 flex items-center justify-center border-2"
                                        >
                                            <IconTrash size={12} />
                                        </button>
                                    </div>
                                ...
                                {mode === 'MANAGE' && (
                                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <div className="w-6 h-6 bg-green-500 border-2 border-green-800 flex items-center justify-center shadow-lg">
                                            <IconPlay size={12} className="text-white ml-0.5" />
                                        </div>
                                    </div>
                                )}
                            </div>
